...
var remote = {
  name: 'origin',
  url: pkg.remoteHtmlUrl,//'http://10.199.133.181:9999/vpal-new/sh_frontend/cashier-h5',
  branch: 'tristan-test',
  website: 'http://10.199.133.181:9999/#/admin/projects/',
  static: true,
};
const temp = 'tempHTML'
const options = {
...
async function deploy() {
  ...
  //console.info('start deploy project:',process.argv)
  //return
  await makeDir(`${temp}`);
  ...
  if (process.argv.includes('--static')) {
    ...
    //   nosort: true,
    //   dot: true,
    //   ignore: [`${temp}/.git`, `${temp}/public`,`${temp}/.gitignore`],
    // });
    await copyDir('release/', `${temp}`);
    await copyFile('release/noah/env/moana_extend_start', `tempHTML/noah/env/moana_extend_start`)
  }
